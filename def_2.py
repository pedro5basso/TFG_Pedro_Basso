...
class Main(tk.Frame):
    ...
    def AskDirectories(self):
        ...
        try:
            ...
            self.URL_D3_FILES =  filedialog.askdirectory()
            file_url = self.URL_D3_FILES + '/directories.txt'

            f = open(file_url,'x') 
            f.close()

            self.URL_IMAGES =  filedialog.askdirectory()
            self.URL_GT =  filedialog.askdirectory()

            self.URL_IMAGES = self.URL_IMAGES + '/'
            ...
            self.URL_XML = self.URL_GT + '/all_topics.xml'
            self.ALL_TOPICS_FILE = '/all_topics.txt'
            self.URL_TXT_FILE = self.URL_GT + self.ALL_TOPICS_FILE

            file_url = self.URL_D3_FILES + '/directories.txt'

            src=open(file_url,"r")
            fline1=self.URL_IMAGES + "\n"
            ...
            src=open(file_url,"r")
            fline2=self.URL_D3_FILES + "\n"
            oline2=src.readlines()
            oline2.insert(0,fline2)
            src.close()
            
            src=open(file_url,"w")
            src.writelines(oline2)
            src.close()

            src=open(file_url,"r")
            fline3=self.URL_GT + "\n"
            oline3=src.readlines()
            oline3.insert(0,fline3)
            src.close()
            
            src=open(file_url,"w")
            src.writelines(oline3)
            src.close()

        except:

            f_url = self.URL_D3_FILES + '/directories.txt'
            ...
            URL_GT  = f.readline()
            URL_D3_FILES =  f.readline()
            URL_IMAGES =  f.readline()

            self.URL_GT =  URL_GT.replace('\n','')
            self.URL_D3_FILES =  URL_D3_FILES.replace('\n','')
            self.URL_IMAGES =  URL_IMAGES.replace('\n','')
    ...
    def BuildAll_topics_txt_file(self):
        ...
        file_topics_url = self.URL_GT + self.ALL_TOPICS_FILE
    ...
    def ResultsWindow(self):
        ...
        if(folder_name != ''):
            ...
            file_all_topics = open(self.URL_TXT_FILE,"r")
            ...
            SolWindow(self.new, num_topic,root.filename,order_clusters,self.URL_IMAGES ,self.URL_D3_FILES ,self.URL_GT)
    ...
    def CheckResultwindow(self):
        ...
        if(file_txt_name != ''):          
            ...
            self.new.geometry(str(screen_width+100) + 'x' + str(screen_height))
            self.new.grid()
            Win5(self.new,file_txt_name,root.filename,self.URL_IMAGES ,self.URL_D3_FILES ,self.URL_GT)
        else:
            print('Choose a folder!')
    ...
    def RunHtml(self): 
        ...
        url_ = self.MakeURLHTTP()
        print(url_)
        webbrowser.open(url_,new=new)

        return
    
    def MakeURLHTTP(self):

        url_images = self.URL_D3_FILES
...
class SolWindow():
    ...
    def __init__(self, master, num_topic,folder_topic,order_clusters, URL_IMAGES ,URL_D3_FILES ,URL_GT):
        ...
        self.MakeURLS(URL_IMAGES ,URL_D3_FILES ,URL_GT)
    ...
    def MakeURLS(self,URL_IMAGES ,URL_D3_FILES ,URL_GT):

        self.URL_IMAGES =  URL_IMAGES
        self.URL_D3_FILES =  URL_D3_FILES
        self.URL_GT =  URL_GT
    ...
    def Read_topics_file(self, num_topic):
        ...
        # print('numero de topico: ', num_topic)
    ...
    def MakeURLDGT(self,list_clus_belong_photos,name_topic):
        ...
        for i in range(0,size_list):
            ...
            image_path = self.URL_IMAGES + name_topic + "/" + id_photo_dgt + ".jpg"
    ...
    def FramesDown(self,images_vector,num_topic, name_topic,Color_List,images_vector_jpg,order_clusters,list_similarity,list_clus_belong_photos):
        ...
        self.GENERAL_CANVAS = Canvas(frame_canvas,width=1510, height=730, bg='white')
        self.GENERAL_CANVAS.grid(row=0, column=0)

        scrollbar = Scrollbar(frame_canvas,orient="vertical",command=self.GENERAL_CANVAS.yview)       
        scrollbar.grid(row=0, column=1, sticky='ns')

        self.GENERAL_CANVAS.bind_all("<MouseWheel>", self._on_mousewheel)

        self.GENERAL_CANVAS.configure(yscrollcommand=scrollbar.set)

        frame_fotos = tk.Frame(self.GENERAL_CANVAS)
        frame_fotos.configure(background='white')
        self.GENERAL_CANVAS.create_window((0,0), window=frame_fotos, anchor='nw')
        ...
        self.GENERAL_CANVAS.config(scrollregion=self.GENERAL_CANVAS.bbox("all"))
...
class Win5():
    ...
    def __init__(self, master,filename,path_filename,URL_IMAGES ,URL_D3_FILES ,URL_GT):
